...
def load_config():
    ...
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configuration.json')
...
def validate_config(config):
    ...
    for script in config['scripts']:
        ...
        if missing_keys:
            raise ValueError(f"Missing required configuration keys {missing_keys} in script: {script.get('name', 'Unknown')}")
        missing_path_keys = [key for key in script['path_order'] if key not in script['paths']]
        if missing_path_keys:
            raise ValueError(f"Missing required path keys {missing_path_keys} in script: {script['name']} based on defined path_order.")
...
def main():
    ...
    while True:
        ...
        extra_args = []
        if chosen_script['name'] == "trimming.sh":
