...
TRIMMED_OUTPUT_DIR=$(jq -r '.scripts[] | select(.name=="align.sh") | .paths.TRIMMED_OUTPUT_DIR' "$CONFIG_FILE")
GENOME_DIR=$(jq -r '.scripts[] | select(.name=="align.sh") | .paths.GENOME_DIR' "$CONFIG_FILE")
OUTPUT_DIR=$(jq -r '.scripts[] | select(.name=="align.sh") | .paths.OUTPUT_DIR' "$CONFIG_FILE")
